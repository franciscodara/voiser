...
class SyncPullService {
  ...
  Future<void> pullAndMerge() async {
    ...
    if (user == null) {
      debugPrint('🔐 [SyncPull] Usuário não autenticado. Abortando pull.');
      return;
    }

    try {
      final metadataService = _ref.read(syncMetadataServiceProvider);
      final lastSyncAt = await metadataService.getLastSyncAt();

      debugPrint('⬇️ [SyncPull] Buscando alterações remotas desde: $lastSyncAt');

      final remoteDatasource = _ref.read(supabaseDatasourceProvider);
      final remoteExpenses = await remoteDatasource.fetchExpenses(updatedAfter: lastSyncAt);

      if (remoteExpenses.isEmpty) {
        debugPrint('📭 [SyncPull] Nenhuma alteração remota encontrada.');
        await metadataService.setLastSyncAt(DateTime.now());
        return;
      }

      debugPrint('📥 [SyncPull] Encontradas ${remoteExpenses.length} alterações remotas.');
      ...
      // Criar mapa para acesso rápido O(1)
      ...
      for (var remote in remoteExpenses) {
        if (remote.deletedAt != null || remote.deleted) {
          // Soft delete remoto vira hard delete local
        ...
        if (!localMap.containsKey(remote.id)) {
          ...
          // Existe localmente: Merge usando Server Wins (updatedAt)
      ...
      // Atualiza metadados com sucesso
      await metadataService.setLastSyncAt(DateTime.now());
      debugPrint('✅ [SyncPull] Merge concluído. Inseridos: $inserted | Atualizados: $updated | Deletados: $deleted');
